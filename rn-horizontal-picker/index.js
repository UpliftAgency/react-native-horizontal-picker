...
export default class HorizontalPicker extends React.Component {
    ...
    onPanResponderRelease: (e, gestureState) => {
      let updateForVelocity = () => {
        if (Math.abs(gestureState.vx) > 1) {
          ...
          const multiplierDueToVelocity = Math.pow(
            Math.max(1.05, Math.abs(gestureState.vx)),
            1.65
          );
          ...
          if (hoveredItem) {
            const currentPositionStart =
              this.state.itemPositions[this.getCurrentItemIndex()] || 0;
            const hoveredPositionStart =
              this.state.itemPositions[hoveredItem.index] || 0;
  ...
  getItemSpacing() {
    return this.props.itemSpacing != null
      ? this.props.itemSpacing
      : defaultItemSpacing;
  }
  ...
  calculateItemPositions() {
    ...
    return items.reduce((positions, _, index) => {
      if (index === 0) {
        ...
        const position =
          positions[positions.length - 1] + prevItemWidth + itemSpacing;
  ...
  calculateTranslateXsToCenterItems(itemPositions) {
    ...
    return items.map((_, index) => {
      ...
      const translateXToCenterSelectedValue =
        -offsetBeforeSelectedItem + (this.state.width - selectedItemWidth) / 2;
  ...
  calculateHoveredItem() {
    ...
    const currentPositionCenter =
      currentPosition + this.getItemWidth(selectedItemIdx) / 2;
  ...
  render() {
    ...
    const cut =
      items.length > VIRTUALIZATION_THRESHOLD
        ? ary => cutArray(ary, selectedItemIdx, VIRTUALIZATION_BUFFER)
        : ary => ary;

    // in case there are no/less-than-needed translateXs cached yet, default each translateX to 0
    const translateXs = items.map(
      i =>
        this.state.translateXs[i.index] != null
          ? this.state.translateXs[i.index]
          : 0
    );
    ...
    const minWidth =
      this.state.itemPositions[this.state.itemPositions.length - 1] +
      this.state.itemWidths[this.state.itemWidths.length - 1];
            ...
            transform: [
              {
                translateX: Animated.add(
                  translateXToCenterSelectedValue,
                  this._swipeDX
                )
              }
            ]
            ...
            return (
              <View
                ...
                style={{
                  ...
                  top: 0,
                  left: 0,
                  transform: [{ translateX: offsetFromLeft }]
                }}
              >
                <InternalItem
                  key={item.value}
                  ...
                  enabled={enabled}
                  onSelect={() =>
                    this.props.onValueChange(item.value, item.index)
                  }
...
function cutArray(ary, point, buffer) {
  return ary.slice(
    Math.max(0, point - buffer),
    Math.min(ary.length - 1, point + buffer + 1)
  );
}
